...
return <div className={`workflow-node ${selected?'selected':''}`} style={{borderTopColor:m.color}}>{d.nodeType!=='start'&&<Handle type="target" position={Position.Left}/>}<div className="node-title"><span style={{color:m.color}}>{m.icon}</span><strong>{d.name||m.label}</strong></div><div className="node-kind">{m.label}</div>{d.nodeType==='start'&&Boolean(d.config?.cron)&&<Tag color="blue">定时执行: {String(d.config.cron)}</Tag>}{(d.nodeType==='h2sql'||d.nodeType==='sqlitesql')&&<div className="node-summary">{Boolean(d.config?.dataSourceId)?'已选择数据源':'未选择数据源'}</div>}{d.nodeType!=='end'&&<Handle type="source" position={Position.Right}/>}</div>}
