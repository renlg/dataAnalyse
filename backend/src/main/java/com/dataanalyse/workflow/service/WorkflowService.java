...
public class WorkflowService {
    ...
    public Map<String,Object> detail(Long id){WorkflowEntity w=getEntity(id);Map<String,Object> m=new LinkedHashMap<>(summary(w));m.put("description",w.getDescription());m.put("nodes",getNodes(id));return m;}
    @Transactional public Map<String,Object> save(Long id,Map<String,Object> body){WorkflowEntity w=id==null?new WorkflowEntity():getEntity(id);String name=str(body.get("name"));if(name==null||name.isBlank())throw new BusinessException(400,"工作流名称不能为空");w.setName(name);w.setDescription(str(body.get("description")));String status=str(body.get("status"));if(status!=null){if(!STATUSES.contains(status))throw new BusinessException(400,"工作流状态不正确");w.setStatus(status);}return summary(workflows.save(w));}
    ...
    @Transactional public List<Map<String,Object>> replaceNodes(Long id,List<Map<String,Object>> body){getEntity(id);Set<String> keys=new HashSet<>();List<WorkflowNodeEntity> saved=new ArrayList<>();
        for(Map<String,Object> item:body){String key=str(item.get("nodeKey")),type=str(item.get("nodeType"));if(key==null||key.isBlank()||!keys.add(key))throw new BusinessException(400,"节点标识不能为空且不能重复");if(!TYPES.contains(type))throw new BusinessException(400,"不支持的节点类型："+type);Map<String,Object> config=map(item.get("config"));if("start".equals(type)){String cron=str(config.get("cron"));if(cron!=null&&!cron.isBlank()){try{CronExpression.parse(cron);}catch(Exception e){throw new BusinessException(400,"cron 表达式不合法");}}}
