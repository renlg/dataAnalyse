...
public class WorkflowScheduler {
    ...
    public synchronized void refresh(Long workflowId){ScheduledFuture<?> old=futures.remove(workflowId);if(old!=null)old.cancel(false);crons.remove(workflowId);Optional<WorkflowEntity> workflow=workflows.findById(workflowId);if(workflow.isEmpty()||!"active".equals(workflow.get().getStatus()))return;try{for(WorkflowNodeEntity n:service.getNodeEntities(workflowId)){if("start".equals(n.getNodeType())){String cron=String.valueOf(service.parseConfig(n).getOrDefault("cron",""));if(!cron.isBlank()){CronTrigger trigger=new CronTrigger(cron);ScheduledFuture<?> future=scheduler.schedule(()->runService.trigger(workflowId,"cron"),trigger);if(future!=null){futures.put(workflowId,future);crons.put(workflowId,cron);}return;}}}}catch(Exception ignored){/* 非法 cron 不影响启动 */}}
