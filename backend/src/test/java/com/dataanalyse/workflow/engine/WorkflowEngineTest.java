...
class WorkflowEngineTest {
    ...
    @Test void exposesContextParamsToPython(){WorkflowService workflows=mock(WorkflowService.class);WorkflowNodeEntity node=node("python","python","算时间窗口");when(workflows.parseConfig(node)).thenReturn(Map.of("code","import json;print(json.dumps({'params': params.get('digestDate'), 'contextParams': contextParams.get('digestDate'), 'context': context.get('digestDate')}))"));WorkflowEngine engine=new WorkflowEngine(workflows,mock(DataSourceService.class),mock(LlmClient.class),new ObjectMapper(),mock(ApiKeyService.class),mock(PasswordCipher.class));Object output=engine.execute(List.of(node),null,Map.of("digestDate","2026-08-30")).output();assertEquals(Map.of("params","2026-08-30","contextParams","2026-08-30","context","2026-08-30"),output);}
